...
-- 2. Find the ID and app name for all apps that were last updated on August 1, 2018
...
-- by the highest rating to lowest rated.
...
-- 8. Find all records with min install not exceeding 50, that have a rating
...
-- 9. Find the names of all apps that are rated less than 3 wuth at least 1000 reviews
...
-- 14. Find all the categories that have more than 300 apps.
SELECT category FROM analytics  
    GROUP BY category
    HAVING COUNT(*) > 300;

-- 15. FInd the app that has the highest proportion of reviews
